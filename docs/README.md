...
id: <SEE LOGIC BELOW>
related: <FROM RULE>
title: <REQUIRED -- FROM RULE, IF RULE HAS NO TITLE FAIL>
...
license: <FROM RULE, ELSE NOT INCLUDED>
tags:
...
1. if no ID, one will be autogenerated using a random UUIDv4 and added to the rule
2. if ID passed in rule, the following logic will apply...

To reduce the risk of future collisions, the rule is renamed with a new ID, and the following is written into the rule;

```
id: <NEW RANDOMLY GENERATED UUID V4>
related:
  - id: <OLD ID>
    type: renamed
```
